...
const validate = ({ required }) => (req, res, next) => {
    ...
    if (required && (!mobileNumber || !mobileNumber.trim() || !password || !password.trim())) {
        res.status(400).jsonp('Mobile Number and password are required');
        return;
    }
    if (mobileNumber && !mobileNumber.match(/(91)(\d){10}\b/)) {
        res.status(400).jsonp('Mobile Number format is invalid');
        return;
    }
...
const create = (req, res, next) => {
    const _a = req.body, { mobileNumber, password } = _a, rest = __rest(_a, ["mobileNumber", "password"]);
    const { db } = req.app;
    if (db == null) {
    ...
    const existingUser = db.get('users').find({ mobileNumber }).value();
    if (existingUser) {
        res.status(400).jsonp('Mobile Number already exists');
        return;
    }
    bcrypt
        ...
        try {
            return db
                .get('users')
                .insert(Object.assign({ mobileNumber, password: hash }, rest))
                .write();
        }
        catch (error) {
        ...
        return new Promise((resolve, reject) => {
            jwt.sign({ mobileNumber }, constants_1.JWT_SECRET_KEY, { expiresIn: constants_1.JWT_EXPIRES_IN, subject: String(user.id) }, (error, accessToken) => {
                if (error)
                    reject(error);
                else
